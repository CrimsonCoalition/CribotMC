...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Многофункциональный бот для Minecraft.')
    ...
    server = config.get(args.world, 'сервер')
    port = config.getint(args.world, 'порт')
    username = config.get(args.world, 'ник')
    password = config.get(args.world, 'пароль')
    logger.info('Захожу на [%s:%d] as [%s]', server, port, username)
